...
class PrismaAdapter extends BaseKeystoneAdapter {
  ...
  async _connect({ rels }) {
    ...
    const url = this.dbSchemaName
      ? `${process.env.DATABASE_URL}?schema=${this.dbSchemaName}`
  ...
  _runPrismaCmd(cmd) {
    // const { schemaPath, dbSchemaName } = this;
    // console.log({ cmd, schemaPath, dbSchemaName })
    const ret = execSync(`yarn prisma ${cmd} --schema ${this.schemaPath}`, {
      env: {
        ...process.env,
        DATABASE_URL: `${process.env.DATABASE_URL}?schema=${this.dbSchemaName}`,
      },
      encoding: 'utf-8',
    });
    // console.log(ret)
